...
{
    // Parse command-line arguments for setup file and number of years
    if (argc < 2)
    {
        fprintf(stderr, "Usage: %s <setup_file> [years]\n", argv[0]);
        fprintf(stderr, "Example: %s Setup.bin 100\n", argv[0]);
        return 1;
    }
    ...
    open_encounter_hdf5("orbital_elements.h5");
    ...
    for (int step = 0; step <= N_steps; step++)
    {
        ...
        if ((step % 1 == 0) && (sim->N < MAX_PARTICLES - 1))
        {
            ...
            // 2. Near perihelion: true anomaly within ±60 degrees of perihelion (f ~ 0)
            double r = sqrt(target.x * target.x + target.y * target.y + target.z * target.z);
            double true_anomaly_deg = fabs(o.f * 180.0 / M_PI);

            // Near perihelion if true anomaly is within ±60° of 0° or 360°
